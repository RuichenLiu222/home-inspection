...
这是西电 2026 年推免考核课题二的实践项目。程序接收一张厨房图片，调用 `SmolVLM-500M-Instruct` 判断画面中是否存在以下问题：
...
这是一个面向考核任务的轻量实验项目，重点是完成可复现的数据处理、提示词对比和结果分析。现有结果能够展示小型视觉语言模型的基本调用方法，也保留了误报、漏报和格式错误等真实问题，便于后续在报告和答辩中分析。
